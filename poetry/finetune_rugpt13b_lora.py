"""
Тренировка модели генерации силлабо-тонической поэзии поверх rugpt13B с использованием LoRa.
"""
import glob
import logging
import os
...
class DataSetArguments:
    output_syllables: Optional[bool] = field(
        metadata={"help": "Силлабо-тоническое представление выходного текста (true) или обычное (false)"}
    )
...
if __name__ == '__main__':
    ...
    if not training_args.output_dir:
        training_args.output_dir = os.path.join(proj_dir, 'tmp', os.path.join(proj_dir, 'tmp', 'verses_rugpt13b_lora'))
    ...
    logger.info('Wrapping model to peft...')
    ...
    printer = MyPrinterCallback(os.path.join(proj_dir, 'tmp', 'finetune_rugpt13b_lora.loss.log'))
    trainer = Trainer(
        model=model,
        args=training_args,
    ...
    logger.info('Start training for local_rank=%d...', training_args.local_rank)
